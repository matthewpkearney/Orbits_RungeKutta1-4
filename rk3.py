...
_T = 140*24*60*60 #86400[s] One week
...
def update_pos(r, v, dt):
    ...
    # print("k1s : " + str(k1s))
    # print("k2s : " + str(k2s))
    r[0] = r[0] + (1/3)*(k1s[0] + k2s[0] + k3s[0])
    r[1] = r[1] + (1/3)*(k1s[1] + k2s[1] + k3s[1])
    return [r[0], r[1]]
...
def update_vel(r, v, dt):
    ...
    k2vel = k2v(r, dt)
    k3vel = k3v(r, v, dt)
    v[0] = v[0] + (1/3)*(k1vel[0] + k2vel[0] +k3vel[0] )
    v[1] = v[1] + (1/3)*(k1vel[1] + k2vel[1] + k3vel[1] )
    return [v[0], v[1]]
 
##store the x,y coordinates
pos_coordinates = open("351coordinates.txt", "w")
#graph shows distance of about 1.5 AU's around the sun
ax = plt.axes(xlim=(-25*AU, 25*AU), ylim =(-10*AU, 10*AU))
ax.set_facecolor('black') #space <- black
plt.title('Earths Orbit Around the Sun - RK3')
plt.xlabel('x-pos')
plt.ylabel('y-pos')
